  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/ml/metrics`)
      ...
      .then(data => {
        ...
        setMlMetrics(data);
        setModels([
          { name: "Transaction Classifier v2.1", type: "Random Forest",   status: "Active",   accuracy: `${data.accuracy}%` },
          { name: "Account Takeover Net",         type: "Neural Network",  status: "Active",   accuracy: `${data.coverage}%` },
          { name: "Geo-Anomaly Detector",         type: "Isolation Forest",status: "Training", accuracy: "--" },
        ]);
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {mlMetrics && (
          <div className="text-xs text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full font-semibold">
            ● Live ML · Last trained {new Date(mlMetrics.lastTrained).toLocaleDateString('en-IN')}
          </div>
        )}
      </div>
